...
@export_category("Kick & Roll Settings")
...
func calculate_view_offset(delta):
	...
	if enable_tilt:
		...
		if enable_damage_kick:
			...
			# damage_ratio = ease(damage_ratio, -2) #easing
...
## Takes a value of 0.0 to 1.0 for strength and tweens that on both horizontal and vertical offsets of the camera over seconds
